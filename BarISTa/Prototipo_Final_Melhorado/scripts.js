...
function add_prod_conta(produto,x,preco){
  var section = document.getElementById('resumo_conta_lista');
  
  var newProd = document.createElement("div");
  var nomeProd = document.createElement("span");
  var precoProd = document.createElement("span");
  
  
  nomeProd.textContent = x + " " + produto;
  newProd.className = "item_resumo_conta";
  
  precoProd.className = "preco_conta";
  precoProd.textContent = preco.toFixed(2) + "€";
  
  newProd.appendChild(nomeProd);
  newProd.appendChild(precoProd);
  section.appendChild(newProd);
  
  document.getElementById("preco_TOTAL_conta").textContent = conta_pay.toFixed(2) + "€";
}

function build_conta() {
  var bot_pagar = document.getElementById("PAGAR");
  bot_pagar.disabled = false;
  bot_pagar.style.color = "White";
  for (p in products) {
    var prod = products[p];
    var x = products_x[p];
    var pr = precos[p];
    if (x > 0)
      add_prod_conta(prod,x,pr);
  }
}

function payAll() {
  $("#janela_conta").fadeOut(700);
  $( ".item_resumo_conta" ).remove();
  document.getElementById("preco_TOTAL_conta").textContent = "0.00€";
  var bot_pagar = document.getElementById("PAGAR");
  bot_pagar.disabled = true;
  bot_pagar.style.color = "#419cbd";
}
...
function toggleHighlight (id1,id2,id3,id4) {
  document.getElementById(id1).style.backgroundColor = "#4DAED1"; //b6d4a6
  document.getElementById(id2).style.display = "block";
  document.getElementById(id3).style.backgroundColor = "#AFD0DB";
